...
function gmm_build_XML()
{
	...
	if (isset($memberContext))
	{
		...
		//THIS IS NO LONGER NEEDED		$div_height = max(isset($modSettings['avatar_max_height_external']) ? $modSettings['avatar_max_height_external'] : 0, isset($modSettings['avatar_max_height_upload']) ? $modSettings['avatar_max_height_upload'] : 0);

		// Is the member allowed so send pm's
...
//			if (!empty($modSettings['onlineEnable']) && !$marker['member']['is_guest'])
			if (!empty($modSettings['onlineEnable'])) //&& !$marker['is_guest'])
				$datablurb .= '				
				<h4>
			...
			<div class="centertext">
				...
				if (!empty($modSettings['show_user_images']) && empty($options['show_no_avatars']) && !empty($marker['avatar']['image']))
					$datablurb .= '
				<div class="centertext"><a href="' . $marker['href'] . '">' . $marker['avatar']['image'] . '</a><br /></div>';
				...
				// groups stars
				$datablurb .= '
					<li class="stars">' . $marker['group_icons'] . '</li>';

				// Show how many posts they have made.
				if (!isset($context['disabled_fields']['posts']))
				$datablurb .= '					
					<li class="postcount">'. $txt['member_postcount']. ': '. $marker['posts']. '</li>';
				...
				$datablurb .= '
					<li class="blurb">' . $marker['blurb'] . '</li>';
	...
	<Placemark id="' . $marker['name'] . '">
		<description>
			...
			if (!empty($modSettings['show_user_images']) && empty($options['show_no_avatars']) && !empty($marker['avatar']['image']))
				echo '
					<li class="avatar">
						<a href="', $marker['href'], '">', $marker['avatar']['image'], '</a>
					</li>';
			...
			// Show the post group if and only if they have no other group or the option is on, and they are in a post group.
			if ((empty($modSettings['hide_post_group']) || empty($marker['group'])) && !empty($marker['post_group']))
				echo '
					<li class="postgroup">', $marker['post_group'], '</li>';
			...
			// Show their personal text?
			if (!empty($modSettings['show_blurb']) && !empty($marker['blurb']))
			echo '
					<li class="blurb">', $marker['blurb'], '</li>';
