...
{
	global $smcFunc, $feedcount, $maxitemcount, $insideitem, $tag, $modSettings;
	...
	// Check if a field expired
		...
		if ($current_time > $lastupdate)
		{
			...
			$smcFunc['db_query']('', "UPDATE {db_prefix}ezp_block_parameters_values as v, {db_prefix}ezp_block_parameters AS p
			...
			$result = $smcFunc['db_query']('', "SELECT COUNT(*) as total FROM {db_prefix}ezp_block_parameters_values as v, {db_prefix}ezp_block_parameters AS p
			
			WHERE v.id_parameter = p.id_parameter AND p.parameter_name = 'lastupdate'  AND v.id_layout = $layoutID
			");
			$countRow = $smcFunc['db_fetch_assoc']($result);
			...
			");
				...
				$smcFunc['db_query']('', "INSERT INTO {db_prefix}ezp_block_parameters_values 
				(id_layout, id_parameter, data)
				VALUES ($layoutID," . $paramRow['id_parameter'] . ",'$safeSQLData')");
...
{
	global $context, $feedcount,$maxitemcount, $insideitem, $tag, $tag_attrs;
		...
		if (empty($encoding))
			$encoding = "ISO-8859-1";
				...
				//	echo htmlspecialchars($feedData,ENT_QUOTES);
					...
					else
					{
						...
						for ($i = 0; $i <= ($numShow -1); $i++)
						{
							...
							if(!isset($context['feeditems'][$i]['title']))
								continue;
								
					
							if ($showBody == true)
							{
								if(!isset($context['feeditems'][$i]['description']))
									continue;
							}
