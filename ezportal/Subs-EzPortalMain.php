...
{
	global $ezpSettings, $db_prefix, $modSettings;
	...
	if (($ezpSettings = cache_get_data('ezpSettings', 90)) == null)
	{
		...
		SELECT
			variable, value
		FROM {$db_prefix}ezp_settings",  __FILE__, __LINE__);
...
{
	global $sourcedir, $ezpSettings, $db_prefix, $modSettings, $func;
	...
	*/
	...
	if (isset($ezBlock['title']))
		$title = $func['htmlspecialchars']($ezBlock['title'],ENT_QUOTES);
	else
	...
	if (isset($ezBlock['author']))
		$author = $func['htmlspecialchars']($ezBlock['author'],ENT_QUOTES);
	else
		$author = '';

	if (isset($ezBlock['website']))
		$website = htmlspecialchars($ezBlock['website'],ENT_QUOTES);
	else
		$website = '';
	...
	if (isset($ezBlock['blockdata']))
		$blockdata = $func['htmlspecialchars']($ezBlock['blockdata'],ENT_QUOTES);
	else
		$blockdata = '';
	...
	db_query("INSERT INTO {$db_prefix}ezp_blocks
			...
			VALUES ('$title', '$blocktype','$version','$author','$website',$can_cache,$editable,'$forumversion','$blockdata')", __FILE__, __LINE__);
	...
	foreach($ezBlock as $key=> $parameter)
	{
		if (substr_count($key,"parameter") != 0)
		{
				...
				if (isset($parameter['default']))
					$pdefault = $func['htmlspecialchars']($parameter['default'],ENT_QUOTES);
				...
				if (isset($parameter['title']))
					$ptitle = $func['htmlspecialchars']($parameter['title'],ENT_QUOTES);
				...
				VALUES ($blockID, '$ptitle', '$ptype','$pdefault', '$prequired', '$key', $porder)", __FILE__, __LINE__);

				$parameterID = db_insert_id();
				...
				if (isset($parameter['selectvalues']))
				{
					...
					VALUES ($blockID, $parameterID, '$paramSelect','$paramSelect')", __FILE__, __LINE__);
...
{
	global $txt, $scripturl, $user_info, $sc, $context, $ID_MEMBER, $modSettings;
		...
		<form action="', $scripturl, '?action=login2" method="post" accept-charset="', $context['character_set'], '">
			<table border="0" cellspacing="1" cellpadding="0">
				...
				<tr>
					...
					<td colspan="2"><input type="submit" value="', $txt['ezp_built_login'], '" /></td>
				</tr>';

global $boardurl, $modSettings;
if (isset($modSettings['slp_apikey']))
...
var login_params=
{
	...
	,containerID: \'componentDiv5\'
	,redirectURL: \'' . $boardurl. '/gigya.php\'
}
</script>
<div id="componentDiv5" align="center"></div>
<script type="text/javascript">
...
// End social login pro
	...
	else
	{
		...
		if (!empty($context['unapproved_members']))
			echo '
								', $context['unapproved_members'] == 1 ? $txt['ezp_approve_thereis'] : $txt['ezp_approve_thereare'], ' <a href="', $scripturl, '?action=viewmembers;sa=browse;type=approve">', $context['unapproved_members'] == 1 ? $txt['ezp_approve_member'] : $context['unapproved_members'] . ' ' . $txt['ezp_approve_members'], '</a> ', $txt['ezp_approve_members_waiting'], '<br />';
		...
		{
			// My Files Link
			echo '<a href="', $scripturl, '?action=downloads;sa=myfiles;u=' . $ID_MEMBER . '">', $txt['ezp_userbox_myfiles'] , '</a><br />';
		}

		if (isset($modSettings['class_set_listings_per_page']))
		{
			// My Listings Link
			echo '<a href="', $scripturl, '?action=classifieds;sa=mylistings;u=' . $ID_MEMBER . '">', $txt['ezp_userbox_mylistings'] , '</a><br />';
		}
...
function EzBlockRecentPostsBlock($parameters = array(), $numPosts = 10, $exclude_boards = null, $format = 'vertical', $startHtml = '', $endHtml = '')
{
	...
	$request = db_query("
		SELECT
			m.posterTime, m.subject, m.ID_TOPIC, m.ID_MEMBER, m.ID_MSG, m.ID_BOARD, b.name AS bName, mg.onlineColor, mg.ID_GROUP,
			...
			echo '
				<tr>
					<td valign="top">
						...
						', $post['new'] ? '' : '<a href="' . $scripturl . '?topic=' . $post['topic'] . '.msg' . $post['new_from'] . ';topicseen#new"><img src="' . $settings['images_url'] . '/' . $context['user']['language'] . '/new.gif" alt="' . $txt['ezp_built_new'] . '" border="0" /></a>', '
						<br />', $post['time'], '
						</span>
						<hr />
					</td>
...
{
	global $txt, $db_prefix, $settings, $modSettings, $context, $modSettings, $user_info, $scripturl;
...
echo '
	...
	foreach($context['available_themes'] as $c => $myTheme)
		echo 'selectThemes[',$myTheme['id'],'] = "' . $myTheme['images_url'] .  '/thumbnail.gif";' . "\n";
	...
	if ($showPreview == 'Yes')
	{
		foreach($context['available_themes'] as $myTheme)
		{
			...
			echo '
		<img id="themePick" src="' . $myTheme['images_url'] .  '/thumbnail.gif" alt="*" />';
...
{
	global $txt, $user_info, $db_prefix;
...
{
	global $txt, $db_prefix, $user_info, $scripturl;
	...
	while ($row = mysql_fetch_assoc($result))
	{
		if (!isset($row['realName']))
			$totalGuests++;
		elseif (!empty($row['showOnline']) || $can_moderate)
		{
			...
			$loggedInUsers[$row['logTime'] . $row['memberName']] = array(
				'id' => $row['ID_MEMBER'],
				'name' => $row['realName'],
				'group' => $row['ID_GROUP'],
...
function EzBlockSMFArticlesEzBlock($parameters = array(), $rows = 4, $articles = 4, $category = 0, $type = 'recent', $startHtml = '', $endHtml = '')
{
	...
	echo '<table cellspacing="0" cellpadding="5" border="0" align="center" width="100%">
			...
			{
				case 'recent':
					$query = "SELECT a.ID_ARTICLE, a.title, a.date, a.rating, a.totalratings, m.realName, a.ID_MEMBER, a.description, a.views, a.commenttotal
				...
				case 'toprated':
					$query = "SELECT
					a.ID_ARTICLE, a.title, a.date, a.rating, a.totalratings, m.realName,
		...
		while($row = mysql_fetch_assoc($dbresult))
		{
			...
			echo '<span class="smalltext">';
				...
				if (!empty($modSettings['smfarticles_disp_membername']))
					if ($row['realName'] != '')
						echo $txt['ezp_gallery_text_by'],' <a href="', $scripturl, '?action=profile;u=', $row['ID_MEMBER'], '">', $row['realName'], '</a><br />';
					else
...
{
	global $txt, $db_prefix, $scripturl,  $modSettings, $scripturl, $boardurl;
	...
	echo '<table cellspacing="0" cellpadding="5" border="0" align="center" width="100%">
			...
			{
				case 'recent':
					$query = "SELECT p.ID_FILE, p.commenttotal, $selectPro p.totalratings, p.rating, p.filesize, p.views, p.title, p.ID_MEMBER, m.realName, p.date, p.description, p.totaldownloads
				...
				case 'toprated':
					$query = "SELECT p.ID_FILE, (p.rating / p.totalratings ) AS ratingaverage , p.commenttotal, $selectPro p.totalratings, p.rating, p.filesize, p.views, p.title, p.ID_MEMBER, m.realName, p.date, p.description, p.totaldownloads
					FROM {$db_prefix}down_file as p
					LEFT JOIN {$db_prefix}members AS m  ON (m.ID_MEMBER = p.ID_MEMBER)
					$selectProJoin
...
function EzBlockGalleryBlock($parameters = array(), $rows = 4, $images = 4, $category = 0, $type = 'recent', $startHtml = '', $endHtml = '')
{
	...
	else
	{
		...
		SELECT
			...
			p.ID_MEMBER, m.realName, p.date
		FROM {$db_prefix}gallery_pic as p
		LEFT JOIN {$db_prefix}members AS m ON (p.ID_MEMBER = m.ID_MEMBER)
		WHERE p.approved = 1 " . ($category != 0 ? ' AND p.ID_CAT = ' . $category : '' ) . "
			ORDER BY p.ID_PICTURE DESC LIMIT $images", __FILE__, __LINE__);
		else
		$dbresult = db_query("
		SELECT
			p.ID_PICTURE, p.commenttotal, p.filesize, p.views, p.thumbfilename, p.title,
		...
		WHERE ((s.private =0 OR s.private IS NULL ) AND (s.password = '' OR s.password IS NULL )  AND p.USER_ID_CAT !=0 AND p.approved =1) OR (p.approved =1 AND p.USER_ID_CAT =0 AND (c.view IS NULL OR c.view =1))
			...
			GROUP by p.ID_PICTURE ORDER BY p.ID_PICTURE DESC LIMIT $images", __FILE__, __LINE__);
		...
		while($row = mysql_fetch_assoc($dbresult))
		{
			$gallery_recent[] = array(
				...
				'profilelink' => ' <a href="' . $scripturl . '?action=profile;u=' . $row['ID_MEMBER'] . '">'  . $row['realName'] . '</a><br />',
			);
		...
		mysql_free_result($dbresult);
			...
			foreach ($gallery_recent as $picture)
			{
				...
				echo '<td align="center"><a href="' . $scripturl . '?action=gallery;sa=view;id=' . $picture['ID_PICTURE'] . '"><img alt="" src="' . $modSettings['gallery_url'] . $picture['thumbfilename']  . '" /></a><br />
...
function EzBlockGalleryRandomImage($parameters = array(), $category = 0, $numimages = 1, $startHtml = '', $endHtml = '')
{
	...
	else
	{
		...
		WHERE " . ($category == 0 ? '' : '  ID_CAT = ' . $category . ' AND ') . " approved = 1 ORDER BY RAND() LIMIT " . $numimages, __FILE__, __LINE__);
		else
	$request = db_query("
		SELECT
		...
		LEFT JOIN {$db_prefix}gallery_usersettings AS s ON (s.ID_MEMBER = m.ID_MEMBER)
		LEFT JOIN {$db_prefix}gallery_catperm AS c ON (c.ID_GROUP IN ($groupsdata) AND c.ID_CAT = p.ID_CAT)
		WHERE ((s.private =0 || s.private IS NULL ) AND (s.password = '' OR s.password IS NULL )  AND p.USER_ID_CAT !=0 AND p.approved =1) OR (p.approved =1 AND p.USER_ID_CAT =0 AND (c.view IS NULL OR c.view =1))
					 " . ($category != 0 ? ' AND p.ID_CAT = ' . $category : '' ) . "
...
{
	global $txt, $db_prefix, $scripturl, $modSettings;
	...
	// Check if links is installed
	 $dbresult = db_query("
		SELECT
			l.ID_LINK,l.title,l.date, l.pagerank, l.alexa, l.rating, m.realName, l.ID_MEMBER, l.description,l.hits
...
{
	global  $db_prefix, $scripturl, $txt, $modSettings, $boardurl, $context;
...
{
	global  $db_prefix, $scripturl, $txt, $modSettings, $user_info, $boardurl, $context, $sourcedir, $boarddir;
	...
	echo '<table cellspacing="0" cellpadding="5" border="0" align="center" width="100%">
			...
			switch($type)
			{
				case 'recent':
					...
					WHERE i.removed = 0 AND i.approved = 1  AND (r.view IS NULL OR r.view =1) GROUP BY i.ID_LISTING  ORDER BY i.ID_LISTING DESC LIMIT $listings";
				break;
				...
				case 'viewed':
					...
					LEFT JOIN {$db_prefix}class_cat as c ON (i.ID_CAT = c.ID_CAT)
					LEFT JOIN {$db_prefix}class_catperm AS r ON (r.ID_GROUP IN ($groupsdata) AND r.ID_CAT = i.ID_CAT)
					WHERE i.removed = 0 AND i.approved = 1  AND (r.view IS NULL OR r.view =1) GROUP BY i.ID_LISTING ORDER BY  i.views DESC LIMIT $listings";
				break;
				...
				case 'random':
					...
					LEFT JOIN {$db_prefix}class_cat as c ON (i.ID_CAT = c.ID_CAT)
					LEFT JOIN {$db_prefix}class_catperm AS r ON (r.ID_GROUP IN ($groupsdata) AND r.ID_CAT = i.ID_CAT)
					WHERE i.removed = 0 AND i.approved = 1  AND (r.view IS NULL OR r.view =1) GROUP BY i.ID_LISTING ORDER BY  RAND() DESC LIMIT $listings";
				break;
				...
				case 'mostcomments':
					...
					LEFT JOIN {$db_prefix}class_cat as c ON (i.ID_CAT = c.ID_CAT)
					LEFT JOIN {$db_prefix}class_catperm AS r ON (r.ID_GROUP IN ($groupsdata) AND r.ID_CAT = i.ID_CAT)
					WHERE i.removed = 0 AND i.approved = 1  AND (r.view IS NULL OR r.view =1) GROUP BY i.ID_LISTING  ORDER BY i.commenttotal DESC LIMIT $listings";
				break;
				...
				case 'featured':
					...
					LEFT JOIN {$db_prefix}class_cat as c ON (i.ID_CAT = c.ID_CAT)
					LEFT JOIN {$db_prefix}class_catperm AS r ON (r.ID_GROUP IN ($groupsdata) AND r.ID_CAT = i.ID_CAT)
					WHERE i.removed = 0 AND i.approved = 1 AND i.featuredlisting = 1  AND (r.view IS NULL OR r.view =1) GROUP BY i.ID_LISTING  ORDER BY i.ID_LISTING DESC LIMIT $listings";
				break;
...
{
	global $settings, $modSettings, $sourcedir, $user_info, $txt, $db_prefix, $scripturl;
	...
	else if ($type == 'latestscores')
	{
		...
		if (count($scores) > 0)
			foreach ($scores as $score) // Print out latest scores
				printf($txt['ezp_arcade_latest_score'], $scripturl . '?action=arcade;sa=play;game=' . $score['game_id'], $score['name'], $score['score'], $score['memberLink'], strip_tags($score['time']));
		else
...
{
	global $txt, $db_prefix, $scripturl, $boardurl, $ezpSettings;
	...
	LIMIT $numToShow", __FILE__, __LINE__);
	while ($row = mysql_fetch_assoc($dbresult))
	{
...
{
	global $scripturl, $db_prefix, $txt, $settings, $modSettings, $context, $boardurl;
	global $func;
	$showlike = false;
	...
	$request = db_query("
		SELECT
			...
			t.numReplies, t.ID_TOPIC, m.ID_MEMBER, m.smileysEnabled, m.ID_MSG, t.locked, mg.onlineColor
	...
	while ($row = mysql_fetch_assoc($request))
	{
		...
		$return[] = array(
			...
			'link' => '<a href="' . $scripturl . '?topic=' . $row['ID_TOPIC'] . '.0">' . $row['numReplies'] . ' ' . ($row['numReplies'] == 1 ? $txt['ezp_built_news_1'] : $txt['ezp_built_news_2']) . '</a>',
			'replies' => $row['numReplies'],
			'comment_href' => !empty($row['locked']) ? '' : $scripturl . '?action=post;topic=' . $row['ID_TOPIC'] . '.' . $row['numReplies'] . ';num_replies=' . $row['numReplies'],
			'comment_link' => !empty($row['locked']) ? '' : '<a href="' . $scripturl . '?action=post;topic=' . $row['ID_TOPIC'] . '.' . $row['numReplies'] . ';num_replies=' . $row['numReplies'] . '">' . $txt['ezp_built_news_3'] . '</a>',
			'new_comment' => !empty($row['locked']) ? '' : '<a href="' . $scripturl . '?action=post;topic=' . $row['ID_TOPIC'] . '.' . $row['numReplies'] . '">' . $txt['ezp_built_news_3'] . '</a>',
			'poster' => array(
				'id' => $row['ID_MEMBER'],
				'name' => $row['posterName'],
				'href' => !empty($row['ID_MEMBER']) ? $scripturl . '?action=profile;u=' . $row['ID_MEMBER'] : '',
				'link' => !empty($row['ID_MEMBER']) ? '<a href="' . $scripturl . '?action=profile;u=' . $row['ID_MEMBER'] . '" style="color: ' . $row['onlineColor'] . ';">' . $row['posterName'] . '</a>' : $row['posterName']
			),
...
function EzBlockShoutBoxBlock($parameters = array(), $numberofShouts = 10, $iframe = false)
{
	...
	echo '<hr />
		...
		if ($ezpSettings['ezp_shoutbox_showbbc'])
		{
			...
			$context['bbc_tags2'][] = array(
				'bold' => array('code' => 'b', 'before' => '[b]', 'after' => '[/b]', 'description' => $txt['ezp_bbc_bold']),
				'italicize' => array('code' => 'i', 'before' => '[i]', 'after' => '[/i]', 'description' => $txt['ezp_bbc_italic']),
				'underline' => array('code' => 'u', 'before' => '[u]', 'after' => '[/u]', 'description' => $txt['ezp_bbc_underline']),
				'strike' => array('code' => 's', 'before' => '[s]', 'after' => '[/s]', 'description' => $txt['ezp_bbc_strike']),
				array(),
				'img' => array('code' => 'img', 'before' => '[img]', 'after' => '[/img]', 'description' => $txt['ezp_bbc_img']),
				'url' => array('code' => 'url', 'before' => '[url]', 'after' => '[/url]', 'description' => $txt['ezp_bbc_link']),
				'move' => array('code' => 'move', 'before' => '[move]', 'after' => '[/move]', 'description' => $txt['ezp_bbc_move']),
			);
			$context['bbc_tags2'][] = array(

				'left' => array('code' => 'left', 'before' => '[left]', 'after' => '[/left]', 'description' => $txt['ezp_bbc_left'] ),
				'center' => array('code' => 'center', 'before' => '[center]', 'after' => '[/center]', 'description' => $txt['ezp_bbc_center']),
				'right' => array('code' => 'right', 'before' => '[right]', 'after' => '[/right]', 'description' => $txt['ezp_bbc_right']),

				array(),
				'size' => array('code' => 'size', 'before' => '[size=10pt]', 'after' => '[/size]', 'description' => $txt['ezp_bbc_size']),
				'face' => array('code' => 'font', 'before' => '[font=Verdana]', 'after' => '[/font]', 'description' => $txt['ezp_bbc_face'] ),
				array(),
				'hr' => array('code' => 'hr', 'before' => '[hr]', 'description' => $txt['ezp_bbc_hr'] ),
	...
	echo '
			<textarea name="shout" id="shout" rows="3" cols="30" style="width: 96%"></textarea>
			<br />
...
{
	global $txt, $db_prefix, $modSettings, $scripturl, $context, $user_info;
...
{
	global $db_prefix, $scripturl, $memberContext, $txt;
	...
	$dbresult = db_query("
		SELECT
			m.ID_MEMBER, m.realName, mg.onlineColor, mg.ID_GROUP
		FROM {$db_prefix}members AS m
		LEFT JOIN {$db_prefix}membergroups AS mg ON (mg.ID_GROUP = IF(m.ID_GROUP = 0, m.ID_POST_GROUP, m.ID_GROUP))
		WHERE m.is_activated = 1
		ORDER BY m.ID_MEMBER DESC LIMIT $numShow", __FILE__, __LINE__);
	...
	while($row = mysql_fetch_assoc($dbresult))
	{
		if ($format == 'vertical')
			...
			echo '<td>
					...
					if ($showColor == true)
					{
						echo '<a href="',$scripturl,'?action=profile;u=',$row['ID_MEMBER'],'"><font color="',$row['onlineColor'],'">',$row['realName'],'</font></a>';
					}
					else
					{
						echo '<a href="',$scripturl,'?action=profile;u=',$row['ID_MEMBER'],'">',$row['realName'],'</a>';
					}
...
{
	global $db_prefix, $scripturl, $txt, $memberContext;
	...
	$dbresult = db_query("
		SELECT
			m.ID_MEMBER, m.realName, mg.onlineColor, mg.ID_GROUP, m.posts
		FROM {$db_prefix}members AS m
		LEFT JOIN {$db_prefix}membergroups AS mg ON (mg.ID_GROUP = IF(m.ID_GROUP = 0, m.ID_POST_GROUP, m.ID_GROUP))

		ORDER BY m.posts DESC LIMIT $numShow", __FILE__, __LINE__);
	echo '<table width="100%">';
...
function EzBlockRSSBlock($parameters = array(), $showBody = false, $numShow = 10, $feedurl = '', $feedData = '', $lastupdate = '',  $updatetime = 15, $format = 'vertical', $newwindow = 0,  $reverseorder = 1, $encoding = 'ISO-8859-1', $startHtml = '', $endHtml = '')
{
	...
	include_once($sourcedir . '/Subs-EzPortalRSS.php');
...
function EzBlockCalenderBlock($parameters = array(), $numShow = 10,  $format = 'vertical', $startHtml = '', $endHtml = '')
{
	global $ezpSettings;
}
...
{
	global $ezpSettings, $db_prefix, $user_info;
	...
	if (($ezblockMenuCache = cache_get_data('ezBlockMenu_' . $ezblocklayoutid, 90)) == null)
	{
		$dbresult = db_query("
			...
			ORDER BY id_order ASC", __FILE__, __LINE__);
		while($row = mysql_fetch_assoc($dbresult))
		{
	...
	foreach($ezblockMenuCache as $row)
	{
		// Display the items
			...
			echo '<div style="float: left; margin-left: -.9em; margin-top: -1.2em;"><ul><li><a href="' . $row['linkurl'] . '" ' . $target . '>' . $row['title'] . '</a></li></ul></div>';
...
function EzBlockParseBBCBlock($parameters = array(), $bbcText = '', $startHtml = '', $endHtml = '')
{
	...
	foreach($parameters as $myparam)
	{
		if ($myparam['parameter_name'] == 'bbctext')
			$bbcText = stripslashes($myparam['data']);
