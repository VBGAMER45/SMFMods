...
{
    global $modSettings, $sourcedir, $boarddir, $boardurl, $db_prefix, $AevaSettings;
    ...
    $result = db_query("SELECT name,value FROM {$db_prefix}aeva_settings", __FILE__, __LINE__);
    $AevaSettings = array();
...
function AevaImportWelcome()
{
 ...
 {
    global $context, $db_prefix, $modSettings, $AevaSettings, $txt;

    // Create Tables needed

    db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_rating
        ...
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_creport
            (ID int(11) NOT NULL auto_increment,
            ID_PICTURE int(11) NOT NULL,
            ...
            db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_usercat
            ...
            PRIMARY KEY  (USER_ID_CAT))", __FILE__, __LINE__);



        //Gallery Member Quota Information
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_userquota
        (ID_MEMBER mediumint(8) unsigned NOT NULL,
        totalfilesize int(12) NOT NULL default '0',
        PRIMARY KEY  (ID_MEMBER))", __FILE__, __LINE__);

        //Gallery Gruop Quota limit
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_groupquota
        (ID_GROUP smallint(5) unsigned NOT NULL default '0',
        totalfilesize int(12) NOT NULL default '0',
        PRIMARY KEY  (ID_GROUP))", __FILE__, __LINE__);


        db_query("ALTER TABLE {$db_prefix}gallery_userquota CHANGE totalfilesize totalfilesize BIGINT NOT NULL  default '0'", __FILE__, __LINE__);
        db_query("ALTER TABLE {$db_prefix}gallery_groupquota CHANGE totalfilesize totalfilesize BIGINT NOT NULL  default '0'", __FILE__, __LINE__);



        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_catperm
        ...
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_usersettings
        (
        ID_MEMBER mediumint(8) unsigned NOT NULL,
        title tinytext,
        password tinytext,
        ...
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_userprivate
        (
        ID_OWNER mediumint(8) unsigned NOT NULL,
        ID_MEMBER mediumint(8) unsigned NOT NULL)", __FILE__, __LINE__);


        // Custom Fields table
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_custom_field
        ...
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_custom_field_data
        (
        ...
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_exif_data
        (
        ID_PICTURE int(11) NOT NULL default '0',
        ...
        db_query("CREATE TABLE IF NOT EXISTS {$db_prefix}gallery_log_mark_view (
          id_member mediumint(8) unsigned NOT NULL default '0',
		...
		db_query("TRUNCATE {$db_prefix}gallery_pic", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_comment", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_cat", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_rating", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_creport", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_userquota", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_groupquota", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_usercat", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_catperm", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_usersettings", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_userprivate", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_custom_field", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_custom_field_data", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_exif_data", __FILE__, __LINE__);
		db_query("TRUNCATE {$db_prefix}gallery_log_mark_view", __FILE__, __LINE__);
...
function AevaImport1()
{
    ...
    // Import Categories
		...
		if (empty($_REQUEST['start']))
		{
			...
			foreach($finalArray as $row)
			{
				...
				VALUES ('" . $row['id_album'] . "','" . addslashes($row['name']) . "','" . addslashes($row['description']) . "','" . $row['parent'] . "','" . $row['a_order'] . "' )", __FILE__, __LINE__);
	...
	$request =db_query("
		SELECT COUNT(*) FROM {$db_prefix}aeva_albums $notInSQL ", __FILE__, __LINE__);
	...
	$request = db_query("
		...
		$notInSQL
		LIMIT " . $_REQUEST['start'] . ","  . ( $increment), __FILE__, __LINE__);

	while ($row = mysql_fetch_assoc($request))
	{
				db_query("INSERT INTO {$db_prefix}gallery_usercat
				(user_id_cat,title,description,id_parent,roworder,id_member)
				VALUES ('" . $row['id_album'] . "','" . addslashes($row['name']) . "','" . addslashes($row['description']) . "','" . $row['parent'] . "','" . $row['a_order'] . "', '" . $row['album_of'] . "')", __FILE__, __LINE__);
	...
	if($_REQUEST['start'] < $totalProcess)
	{
		...
		{
				db_query("INSERT IGNORE INTO {$db_prefix}gallery_cat
				(title, description,roworder,ID_PARENT,redirect,image)
			VALUES ('Member Galleries', 'Galleries created by members',0,0,1,'')", __FILE__, __LINE__);
...
commenttotal, approved, date, keywords,allowcomments )
					...
					'" . $row['filesize'] .  "', '" . $row['height'] . "', '" . $row['width'] . "',
					 ...
					 , '" . addslashes($row['keywords']) ."',1)", __FILE__, __LINE__);
...
function AevaImport2b()
{
    ...
    $complete = 0;
		...
		$result = db_query("SELECT ID_CAT FROM {$db_prefix}gallery_cat
				where redirect = 1", __FILE__, __LINE__);
...
LEFT JOIN {$db_prefix}aeva_files as p on (p.id_file = m.id_preview)
		...
		$requestCount = db_query("select count(*) as total from {$db_prefix}gallery_pic where id_picture = " .$row['id_media'], __FILE__, __LINE__);
		$picCount = mysql_fetch_assoc($requestCount);
				if ($picCount['total'] > 0)	
					$skipEntry  = true;
				...
				if ($skipEntry == false)
				{
	                ...
	                if (!empty($row['embed_url']))
	                {
						...
						db_query("INSERT IGNORE INTO {$db_prefix}gallery_pic
...
function AevaImport3()
{
    ...
    $complete = 0;
		...
		// Determine the total members with posts.
		$request =db_query("
			SELECT COUNT(*) FROM {$db_prefix}aeva_log_ratings ", __FILE__, __LINE__);
		list($totalProcess) = mysql_Fetch_row($request);
		mysql_free_result($request);
		...
		while ($row = mysql_fetch_assoc($request))
		{
					db_query("INSERT INTO {$db_prefix}gallery_rating
					(id_picture,id_member, value)
					VALUES ('" . $row['id_media'] . "','" . $row['rating']. "','3')", __FILE__, __LINE__);
...
function AevaImport4()
{
	...
	// Determine the total members with posts.
	$request =db_query("
		SELECT COUNT(*) FROM {$db_prefix}aeva_comments ", __FILE__, __LINE__);
	list($totalProcess) = mysql_Fetch_row($request);
	mysql_free_result($request);
	...
	while ($row = mysql_fetch_assoc($request))
	{
				...
				'" . addslashes($row['message']). "','" . $row['posted_on']. "','" . $row['approved']. "')", __FILE__, __LINE__);
...
function AevaImport5()
{
	...
	$request =db_query("
		...
		WHERE p.id_picture = m.id_media ", __FILE__, __LINE__);
	list($totalProcess) = mysql_Fetch_row($request);
	mysql_free_result($request);
	...
	while ($row = mysql_fetch_assoc($request))
	{
				db_query("INSERT INTO {$db_prefix}gallery_log_mark_view
				(id_picture,id_member, id_cat, user_id_cat)
				VALUES ('" . $row['id_media'] . "','" . $row['id_member']. "',
				'" . $row['id_cat']. "','" . $row['user_id_cat']. "')", __FILE__, __LINE__);
...
function AevaImport6()
{
    global $txt, $context, $db_prefix;
		...
		$dbresult = db_query("SELECT ID_CAT FROM {$db_prefix}gallery_cat", __FILE__, __LINE__);
		...
		$dbresult =db_query("SELECT USER_ID_CAT FROM {$db_prefix}gallery_usercat", __FILE__, __LINE__);
		while($row = mysql_fetch_assoc($dbresult))
		{
...
function UpdateCategoryTotals2($ID_CAT)
{
	...
	$dbresult =db_query("SELECT COUNT(*) AS total FROM {$db_prefix}gallery_pic WHERE ID_CAT = $ID_CAT AND approved = 1", __FILE__, __LINE__);
	...
	// Update the count
	//$dbresult =db_query("UPDATE {$db_prefix}gallery_cat SET total = $total WHERE ID_CAT = $ID_CAT LIMIT 1", __FILE__, __LINE__);
...
function UpdateUserCategoryTotals2($USER_ID_CAT)
{
	...
	$dbresult =db_query("SELECT COUNT(*) AS total FROM {$db_prefix}gallery_pic WHERE USER_ID_CAT = $USER_ID_CAT AND approved = 1", __FILE__, __LINE__);
	$row = mysql_fetch_assoc($dbresult);
	$total = $row['total'];
	mysql_free_result($dbresult);
	...
	// Update the count
	$dbresult =db_query("UPDATE {$db_prefix}gallery_usercat SET total = $total WHERE USER_ID_CAT = $USER_ID_CAT LIMIT 1", __FILE__, __LINE__);
...
function ProcessEXIFData($filename,$pictureid)
{
	...
	'" . @$exifData['EXIF']['WhiteBalance']. "','" . @$exifData['EXIF']['SceneCaptureType']. "')", __FILE__, __LINE__);
