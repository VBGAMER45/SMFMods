...
function TaggingSettingsMain()
{
	...
	if (!empty($_POST['save']))
	{
		$tagging_settings = array(
			...
			'tag_delimiters' => !empty($_POST['tag_delimiters']) ? ($_POST['tag_delimiters'] == 'comma') ? 'comma' : 'space' : '',
...
function TaggingSettingsListCloud()
{
	...
	if (!empty($_POST['save']))
	{
		...
		updateSettings($tagging_settings);
		cache_put_data('tag_cloud_cache', null);
		cache_put_data('tag_list_cache', null);
...
function tagList()
{
	...
	if (($context['tagsList'] = cache_get_data('tag_list_cache', 3600)) == null)
	{
		$request = $smcFunc['db_query']('', '
		...
		$smcFunc['db_free_result']($request);
		cache_put_data('tag_list_cache', $context['tagsList'], 3600);	
	}
}
...
function tagCloud()
{
	...
	if (cache_get_data('tag_cloud_cache', 3600) == null || cache_get_data('tag_maximum_cache', 3600) == null)
	{
		$context['terms'] = array();
		$maximum = 0;
		...
		cache_put_data('tag_cloud_cache', $context['terms'], 3600);		
		cache_put_data('tag_maximum_cache', $context['maximum'], 3600);		
	}
	else
	{
		$context['terms'] = cache_get_data('tag_cloud_cache', 3600);
		$context['maximum'] = cache_get_data('tag_maximum_cache', 3600);
	}
}
...
function postTags()
{
	...
	if (!empty($modSettings['tag_enabled']) && !empty($_POST['tags_news']) && !empty($context['active_tags_boards']))
	{
...
//Tagging System Mod - START
function getTagCloud()
{
	global $context, $modSettings, $scripturl, $txt, $sourcedir;
