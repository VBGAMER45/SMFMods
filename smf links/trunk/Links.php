...
function view()
{
	...
	if (isset($_REQUEST['cat']))
	{
		...
		$dbresult = db_query("
			SELECT l.ID_LINK, l.title,l.date, l.pagerank, l.alexa, l.rating, m.realName,
			...
			WHERE l.ID_CAT = $cat
				AND l.approved = 1
			ORDER BY $sort $sorto
			LIMIT $context[start]," . $modSettings['smflinks_setlinksperpage'], __FILE__, __LINE__);
	...
	else
	{
		...
		// Top 5 rated links
		$dbresult = db_query("
			SELECT l.ID_LINK, l.rating, l.title,
				l.date, m.realName, l.ID_MEMBER, l.description, l.hits 
			FROM {$db_prefix}links AS l 
				LEFT JOIN {$db_prefix}members AS m ON (l.ID_MEMBER = m.ID_MEMBER)  
			WHERE l.approved = 1
...
function AddCat2()
{
	...
	if (!empty($_POST['title']))
		$title = trim($func['htmlspecialchars']($_POST['title'], ENT_QUOTES));
	if (empty($title))
		fatal_error($txt['smflinks_nocattitle'], false);

	// Category Description; Optional
	if (!empty($_REQUEST['description']))
		$description = trim($func['htmlspecialchars']($_REQUEST['description'], ENT_QUOTES));
	else
		$description = '';

	// Category Image; Optional
	if (!empty($_POST['image']))
		$image = trim(htmlspecialchars($_POST['image'], ENT_QUOTES));
	else
		$image = '';
	...
	// Insert the category
	db_query("
		INSERT INTO {$db_prefix}links_cat
			(title, description, roworder, image, ID_PARENT)
		VALUES
			('$title', '$description', $order, '$image', $parent)", __FILE__, __LINE__);
	mysql_free_result($dbresult);
...
function AddLink2()
{
	...
	$dbresult = db_query("
		...
		WHERE l.url = '$url'
			AND l.ID_CAT = c.ID_CAT", __FILE__, __LINE__);
	$row = mysql_fetch_assoc($dbresult);
	...
	db_query("
		INSERT INTO {$db_prefix}links
			(ID_CAT, url, title, description, ID_MEMBER, date, approved, alexa, pagerank)
		VALUES
			($catid, '$url', '$title', '$description', $ID_MEMBER, $t, $approved, $alexa, $pagerank)", __FILE__, __LINE__);
...
function CatUp()
{
	...
	$dbresult = db_query("
		...
		AND roworder = $o", __FILE__, __LINE__);
	if (db_affected_rows()== 0)
...
function ApproveList()
{
	...
	$dbresult = db_query("
		SELECT l.ID_LINK, l.approved, l.description, l.pagerank,
			l.alexa, l.url, l.title, l.date, m.realName, l.ID_MEMBER,
			l.description,l.hits, l.ID_CAT, c.title catname 
		FROM ({$db_prefix}links AS l, {$db_prefix}links_cat AS c) 
			LEFT JOIN {$db_prefix}members AS m ON (l.ID_MEMBER = m.ID_MEMBER) 
...
function RateLink()
{
	...
	$dbresult = db_query("
		...
		WHERE ID_MEMBER = $ID_MEMBER
			AND ID_LINK = $id", __FILE__, __LINE__);
	if (db_affected_rows()!= 0)
	...
	if ($value == 0)
	{
		...
		//Insert rating
		db_query("
			INSERT INTO {$db_prefix}links_rating
				(ID_LINK, ID_MEMBER, value)
			VALUES
				($id, $ID_MEMBER, 0)", __FILE__, __LINE__);
...
function LinksAdminPerm()
{
	...
	$dbresult = db_query("
		SELECT c.ID_CAT, c.ID, c.view, c.addlink,
			c.editlink, c.dellink, c.ID_GROUP, m.groupName, a.title catname 
  		FROM ({$db_prefix}links_catperm AS c, {$db_prefix}membergroups AS m,{$db_prefix}links_cat AS a) 
  		WHERE c.ID_GROUP = m.ID_GROUP
...
function GetCatPermission($cat, $perm)
{
	...
	$cat = (int) $cat;
	if (empty($cat))
		fatal_error($txt['smflinks_nocatselected'], true);
...
function CatPerm()
{
	...
	$dbresult = db_query("
		SELECT c.ID_CAT, c.ID, c.view, c.addlink,
			c.editlink, c.dellink,  c.ID_GROUP, m.groupName,a.title catname 
		FROM ({$db_prefix}links_catperm AS c, {$db_prefix}membergroups AS m,{$db_prefix}links_cat AS a) 
 		WHERE c.ID_CAT = " . $context['links_cat'] . "
			AND c.ID_GROUP = m.ID_GROUP
			AND a.ID_CAT = c.ID_CAT", __FILE__, __LINE__);
	...
	$dbresult = db_query("
		SELECT c.ID_CAT, c.ID, c.view, c.addlink, c.editlink, c.dellink, c.ID_GROUP, a.title catname 
		FROM ({$db_prefix}links_catperm AS c,{$db_prefix}links_cat AS a) 
		WHERE c.ID_CAT = " . $context['links_cat'] . "
			AND c.ID_GROUP = 0
			AND a.ID_CAT = c.ID_CAT
...
function CatPerm2()
{
	...
	// Valid some variables.
	if (isset($_REQUEST['groupname']))
		$groupname = (int) $_REQUEST['groupname'];
	if (!empty($_REQUEST['cat']))
		$cat = (int) $_REQUEST['cat'];
	if (empty($cat) || !isset($groupname))
		fatal_error($txt['smflinks_nocatselected'], false);
	...
	db_query("
		INSERT INTO {$db_prefix}links_catperm
			(ID_GROUP, ID_CAT, view, addlink, editlink, dellink)
		VALUES
			($groupname, $cat, $view, $add, $edit, $delete)", __FILE__, __LINE__);
...
function GetLinkTotals($ID_CAT)
{
	...
	if (!empty($modSettings['smflinks_set_count_child']))
	{
		$dbresult3 = db_query("
			SELECT ID_CAT, title
			FROM {$db_prefix}links_cat 
	...
	if (db_affected_rows() != 0)
	{
		echo '
			...
			while ($row = mysql_fetch_assoc($dbresult))
			{
				...
				{
					echo '
						<td class="windowbg2"><a href="' . $scripturl . '?action=links;cat=' . $row['ID_CAT'] . '"><img src="' . $row['image'] . '" border="0" /></a></td>
						<td class="windowbg2"><a href="' . $scripturl . '?action=links;cat=' . $row['ID_CAT'] . '">' . parse_bbc($row['title']) . '</a></td><td class="windowbg2">' . parse_bbc($row['description']) . '</td>
					';
				}
