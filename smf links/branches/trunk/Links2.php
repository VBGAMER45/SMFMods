...
function view()
{
	...
	if (isset($_REQUEST['cat']))
	{
		...
		WHERE ID_CAT = $cat  AND approved = 1");
		$row = $smcFunc['db_fetch_assoc']($dbresult);
		...
		$dbresult = $smcFunc['db_query']('', "
		SELECT 
			l.ID_LINK,l.title,l.date, l.pagerank, l.alexa, l.rating, m.real_name, l.ID_MEMBER, l.description,l.hits FROM {db_prefix}links AS l 
		LEFT JOIN {db_prefix}members AS m  ON (l.ID_MEMBER = m.ID_MEMBER) 
		WHERE l.ID_CAT = $cat  AND l.approved = 1 ORDER BY $sort $sorto LIMIT $context[start]," . $modSettings['smflinks_setlinksperpage']);
	...
	else
	{
		...
		FROM {db_prefix}links_cat 
		WHERE ID_PARENT = 0 ORDER BY roworder ASC");
		...
		// Show top five hits
			...
			FROM {db_prefix}links AS l 
				LEFT JOIN {db_prefix}members AS m  ON (l.ID_MEMBER = m.ID_MEMBER) 
			WHERE l.approved = 1 ORDER BY l.hits DESC LIMIT 5");
		...
		$dbresult = $smcFunc['db_query']('', "
			SELECT 
				COUNT(*) AS total 
			FROM {db_prefix}links WHERE approved = 1");
		$row = $smcFunc['db_fetch_assoc']($dbresult);
...
function DeleteCat2()
{
	...
	// Delete All links
	$smcFunc['db_query']('', "DELETE FROM {db_prefix}links WHERE ID_CAT = $catid");
	// Finally delete the category
	$smcFunc['db_query']('', "DELETE FROM {db_prefix}links_cat WHERE ID_CAT = $catid LIMIT 1");
...
function AddLink()
{
	...
	SELECT
		ID_CAT, title, roworder
	FROM {db_prefix}links_cat ORDER BY roworder ASC");
...
function AddLink2()
{
	...
	WHERE l.url = '$url' AND l.ID_CAT = c.ID_CAT");
	$row = $smcFunc['db_fetch_assoc']($dbresult);
	...
	$smcFunc['db_query']('', "INSERT INTO {db_prefix}links
			(ID_CAT, url, title, description,ID_MEMBER,date,approved,alexa,pagerank)
		VALUES ($catid,'$url','$title', '$description'," . $user_info['id'] . ",$t,$approved,$alexa,$pagerank)");
...
function DeleteLink2()
{
	...
	$smcFunc['db_query']('', "DELETE FROM {db_prefix}links WHERE ID_LINK = $id LIMIT 1");
...
function VisitLink()
{
	...
	// Update site lists
	$smcFunc['db_query']('', "UPDATE {db_prefix}links
		SET hits = hits + 1 WHERE ID_LINK = $id LIMIT 1");
...
function ApproveList()
{
	...
	$dbresult = $smcFunc['db_query']('', "
 SELECT 
 	l.ID_LINK, l.approved, l.description, l.pagerank, l.alexa, l.url, l.title, l.date, m.real_name, l.ID_MEMBER, l.description,l.hits, 
 	l.ID_CAT, c.title catname 
 	FROM ({db_prefix}links AS l, {db_prefix}links_cat AS c) 
 	LEFT JOIN {db_prefix}members AS m ON (l.ID_MEMBER = m.ID_MEMBER) 
 	WHERE l.ID_CAT = c.ID_CAT AND l.approved = 0 ORDER BY l.ID_LINK DESC LIMIT $context[start],20");
...
function Approve()
{
	...
	$smcFunc['db_query']('', "UPDATE {db_prefix}links
		SET approved = 1 WHERE ID_LINK = $id LIMIT 1");
...
function NoApprove()
{
	...
	$smcFunc['db_query']('', "UPDATE {db_prefix}links
		SET approved = 0 WHERE ID_LINK = $id LIMIT 1");
...
function RateLink()
{
	...
	WHERE ID_MEMBER = " . $user_info['id'] . " AND ID_LINK = $id");
	if ($smcFunc['db_affected_rows']()!= 0)
	...
	if ($value == 0)
	{
		// Lower Ranking
			...
			//Insert rating
		$smcFunc['db_query']('', "INSERT INTO {db_prefix}links_rating
				(ID_LINK, ID_MEMBER,value)
			VALUES ($id, " . $user_info['id'] . ",0)");

		// Update main link rating
		$smcFunc['db_query']('', "UPDATE {db_prefix}links
			SET rating = rating - 1 WHERE ID_LINK = $id LIMIT 1");
...
function LinksAdminCats()
{
	...
	$dbresult = $smcFunc['db_query']('', "
		...
		FROM {db_prefix}links_cat ORDER BY ID_CAT DESC, roworder ASC");
	$context['links_cats'] = array();
	while ($row = $smcFunc['db_fetch_assoc']($dbresult))
		$context['links_cats'][] = $row;
	$smcFunc['db_free_result']($dbresult);
...
function LinksAdminPerm()
{
	...
	$dbresult = $smcFunc['db_query']('', "
		  	SELECT 
		  		c.ID_CAT, c.ID, c.view, c.addlink, c.editlink, c.dellink,   c.ID_GROUP, m.group_name,a.title catname 
...
function CatPermDelete()
{
	...
	$smcFunc['db_query']('', "DELETE FROM {db_prefix}links_catperm WHERE ID = " . $id . ' LIMIT 1');
...
function CatPerm()
{
	...
	$dbresult = $smcFunc['db_query']('', "
		  	SELECT 
		  		c.ID_CAT, c.ID, c.view, c.addlink, c.editlink, c.dellink,  c.ID_GROUP, m.group_name,a.title catname 
		  	FROM ({db_prefix}links_catperm as c, {db_prefix}membergroups AS m,{db_prefix}links_cat as a) 
		  	WHERE  c.ID_CAT = " . $context['links_cat'] . " AND c.ID_GROUP = m.ID_GROUP AND a.ID_CAT = c.ID_CAT");
	...
	$dbresult = $smcFunc['db_query']('', "
		  	SELECT 
		  		c.ID_CAT, c.ID, c.view, c.addlink, c.editlink, c.dellink, c.ID_GROUP,a.title catname 
		  	FROM ({db_prefix}links_catperm as c,{db_prefix}links_cat as a) 
		  	WHERE c.ID_CAT = " . $context['links_cat'] . " AND c.ID_GROUP = 0 AND a.ID_CAT = c.ID_CAT LIMIT 1");
	...
	$dbresult = $smcFunc['db_query']('', "
		  	SELECT 
		  		c.ID_CAT, c.ID, c.view, c.addlink, c.editlink, c.dellink,  c.ID_GROUP,a.title catname 
		  	FROM {db_prefix}links_catperm as c,{db_prefix}links_cat as a 
		  	WHERE c.ID_CAT = " . $context['links_cat'] . " AND c.ID_GROUP = -1 AND a.ID_CAT = c.ID_CAT LIMIT 1");
	$context['links_guests_groups'] = array();
	while($row = $smcFunc['db_fetch_assoc']($dbresult))
	{
		$context['links_guests_groups'][] = $row;
...
function CatPerm2()
{
	...
	WHERE ID_GROUP = $group_name AND ID_CAT = $cat");
	if ($smcFunc['db_affected_rows']()!= 0)
	{
		$smcFunc['db_free_result']($dbresult);
	...
	$smcFunc['db_query']('', "INSERT INTO {db_prefix}links_catperm
			(ID_GROUP,ID_CAT,view,addlink,editlink,dellink)
		VALUES ($group_name,$cat,$view,$add,$edit,$delete)");
...
function DoLinksAdminTabs($overrideSelected = '')
{
	...
	SELECT
		COUNT(*) AS total
	FROM {db_prefix}links WHERE approved = 0");
	$row = $smcFunc['db_fetch_assoc']($dbresult);
